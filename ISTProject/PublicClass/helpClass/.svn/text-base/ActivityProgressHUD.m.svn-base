...
#import "ActivityProgressHUD.h"

@interface ActivityProgressHUD ()
...
@implementation ActivityProgressHUD
...
+ (ActivityProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	ActivityProgressHUD *hud = [[ActivityProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return [hud autorelease];//[self layoutSubviews];  //加入指示器的标题   yaozhong
}

+ (ActivityProgressHUD *)showHUDAddedTo:(UIView *)view andTitle:(NSString *)hudTitle animated:(BOOL)animated {
	ActivityProgressHUD *hud = [[ActivityProgressHUD alloc] initWithView:view];
    hud.labelText = hudTitle;
    [hud layoutSubviews];  //加入指示器的标题   yaozhong
	[view addSubview:hud];
	[hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[ActivityProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		ActivityProgressHUD *HUD = (ActivityProgressHUD *)viewToRemove;
...
- (void)done {
    ...
    if(delegate != nil && [delegate conformsToProtocol:@protocol(ActivityProgressHUDDelegate)]) {
